...
if (-not $SharePointOnly) {
    ...
    Remove-AzureRmResourceGroup -Name $ResourceGroupName -Force -Confirm:$false
} else {
    ...
    $CleanUpDetails.SharePointServers | ForEach-Object {
        Remove-AzureRmVM -ResourceGroupName $ResourceGroupName -Name $_ -Force -ErrorAction SilentlyContinue | Out-Null
    }

    Write-Output -InputObject "Task 2: Removing SharePoint Network Interfaces"
    $CleanUpDetails.NetworkInterfaces  | ForEach-Object {
        Remove-AzureRmNetworkInterface -ResourceGroupName $ResourceGroupName -Name $_ -Force -ErrorAction SilentlyContinue | Out-Null
    }

    Write-Output -InputObject "Task 3: Removing SharePoint Public IPs"
    $CleanUpDetails.PublicIPs  | ForEach-Object {
        Remove-AzureRmPublicIpAddress -ResourceGroupName $ResourceGroupName -Name $_ -Force -ErrorAction SilentlyContinue | Out-Null
    }
    ...
    $CleanUpDetails.Blobs | ForEach-Object {
        try
        {
            $key = (Get-AzureRmStorageAccountKey -ResourceGroupName $ResourceGroupName -Name $_.Account).Key1
