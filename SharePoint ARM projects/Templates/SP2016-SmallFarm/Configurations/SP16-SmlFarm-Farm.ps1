...
Configuration SharePointFarmServer
{
    ...
    node "localhost"
    {
        ...
        #
        # This section contains the search specific configuration
        #**********************************************************

		WaitForAll WaitForAllFarmServers
		{
            ResourceName = "[xSPJoinFarm]JoinSPFarm"
            NodeName = @("$ServerNamePrefix-sp2","$ServerNamePrefix-sp3","$ServerNamePrefix-sp4")
            RetryIntervalSec = 60
			RetryCount = 30
            PsDscRunAsCredential = $SPSetupAccount
            DependsOn = @("[xSPServiceAppPool]MainServiceAppPool", "[xSPSite]TeamSite")
		}
        ...
        {  
            ...
            ApplicationPool       = "SharePoint Service Applications"
            PsDscRunAsCredential  = $SPSetupAccount
            DependsOn             = "[WaitForAll]WaitForAllFarmServers"
        }
